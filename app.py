...
# Initialize OCR reader as a cached resource
@st.cache_resource
def get_ocr_reader():
    """Initialize and cache the OCR reader"""
    try:
        # Initialize with English language
        # You can add more languages if needed: reader = easyocr.Reader(['en', 'es'])
...
def pdf_to_images(uploaded_file):
    ...
    try:
        ...
        for page_num in range(len(pdf_document)):
            ...
            # Render page to an image (matrix defines the resolution)
...
def extract_text_with_ocr(uploaded_file):
    ...
    try:
        ...
        with st.spinner("Converting PDF to images for OCR processing..."):
        ...
        for idx, img in enumerate(images):
            ...
            st.text(f"Processing page {idx + 1} of {len(images)}...")
...
def extract_text_from_pdf(uploaded_file):
    ...
    try:
        ...
        # If text is too short or mostly whitespace, it might be a scanned PDF
        if len(cleaned_text) < 100:  # Threshold for minimum text
            st.warning("⚠️ Limited text extracted with standard method. Attempting OCR...")
            ...
            if ocr_text and len(ocr_text.strip()) > len(cleaned_text):
                ...
                st.warning("OCR did not improve text extraction. Using original text.")
                return text
        ...
        st.info("Attempting OCR extraction as fallback...")
        ...
        if ocr_text:
            ...
            st.error("Both standard and OCR extraction failed.")
            return ""

def extract_and_process_resume(uploaded_file):
    """Extract and process resume (with fresh cache detection) - supports PDF and DOCX with OCR"""
    ...
    st.info(f"📄 Extracted {len(resume_text)} characters from the resume")
...
def main():
    ...
    st.markdown("*Powered by Smart Hiring Criteria Pre-screening, parallel processing, and OCR support*")
    ...
    if ocr_reader:
        st.success("✅ OCR Engine Ready (EasyOCR loaded)")
    else:
        st.warning("⚠️ OCR Engine not available - will use standard text extraction only")
    ...
    if uploaded_file is not None:
        ...
        # Extract resume text using modified function (with fresh cache detection and OCR)
    ...
    with st.sidebar:
        ...
        if ocr_reader:
            ...
            st.warning("⚠️ OCR engine: Not available")
